// skins/sim.js — the "Sim city" skin: the old locally-drawn stylised city.
//
// Kept frozen as the third skin so the offline/replay demo and the original
// tests survive the real-map skin (skins/gmaps.js). Same game model as before:
// a weighted city with invented congestion 1–5; nothing here is real geography.
...
export const simSkin = {
  ...
  mount({ container, autoAsk, resultEl }) {
    ...
    wrap.innerHTML = `
      ...
      <div class="map-hint">The SIMULATED city: invented roads, invented congestion 1–5. The real map (with real roads and real place names) lives in the Navigation skin.</div>`;
  ...
  draw(progress) {
    ...
    if (pts.length > 1 && progress > 0) {
      ...
      line('#0b0c10', Math.max(4, cell * 0.34));
      line('#38bdf8', Math.max(2.5, cell * 0.22));
    }
  ...
  drawPins(ctx, cx, cy, cell) {
    ...
    drawPin(cx(src.c), cy(src.r), '#38bdf8');
