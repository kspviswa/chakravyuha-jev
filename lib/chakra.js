 ...
 *
 * Rings 1..R sit at the middle of their band: ring i is at (i - 0.5) bands out.
 * Ring 0 is the centre — a POINT at the origin, not a band — so its radius is 0.
 * The formula alone gives -0.5 bands there, and a negative radius does not mean
 * "at the centre": it means the same distance on the OPPOSITE side. That put the
 * centre cell diametrically across the board from the gate, so the final hop was
 * drawn as a chord sweeping half the innermost ring and cutting straight through
 * its walls.
 *
 * The clamp also covers the FRACTIONAL rings the animator feeds in mid-hop: a hop
 * into the centre interpolates ring 1 → 0, and without the clamp the sprite would
 * swing out to the antipodal side at ring 0.5 and snap back at ring 0.
 */
export function centreRadius(ring, totalRadius, numRings) {
  return Math.max(0, ((ring - 0.5) / numRings) * totalRadius);
}
