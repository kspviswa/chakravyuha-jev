...
// ---- canvas drawing -------------------------------------------------------
// Path2D caches, built lazily. Path2D accepts SVG *path data* — the `d`
// attribute's contents — and NOT an XML element. Lucide markup mixes <path>
// and <circle>, so both must be converted to raw path commands first.
const _paths = new Map();

/** Turn a Lucide element's inner markup into a single SVG path-data string. */
export function pathDataOf(name) {
  const markup = ICONS[name] || '';
  const parts = [];

  // <path d="…"/> → its d attribute verbatim
  for (const m of markup.matchAll(/<path\s[^>]*\bd="([^"]+)"/g)) parts.push(m[1]);

  // <circle cx cy r/> → two half-arcs (a full circle cannot be one arc command)
  for (const m of markup.matchAll(/<circle\s+cx="([^"]+)"\s+cy="([^"]+)"\s+r="([^"]+)"/g)) {
