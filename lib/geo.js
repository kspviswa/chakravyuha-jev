...
// ---- real-road geometry for a grid route -----------------------------------
/**
 * Build a graph from the real road polylines: nodes are road vertices, edges are
 * the segments between consecutive vertices. Used to turn a grid route into a
 * polyline that follows the actual roads — the maze is a grid, but the answer
 * should LOOK like a map route, not a staircase between cell centres.
 */
export function buildRoadGraph(roads) {
  ...
  for (const road of roads || []) {
    const pts = road?.points || [];
    for (let i = 1; i < pts.length; i++) {
      const a = idOf(pts[i - 1][0], pts[i - 1][1]);
      const b = idOf(pts[i][0], pts[i][1]);
      if (a === b) continue;
      adj[a].push(b);
      adj[b].push(a);
    }
...
/** BFS shortest path (hops) between two nodes; null when it exceeds `maxHops`. */
function bfsPath(graph, from, to, maxHops) {
  ...
  for (let h = 0; h < q.length; h++) {
    ...
    for (const v of graph.adj[u]) {
      ...
      if (v === to) {
        const path = [];
        for (let x = to; x !== -1; x = prev.get(x)) path.push(x);
        return path.reverse();
      }
      q.push(v);
    }
...
/** Road vertex nearest a cell's centre (ties broken by the lower id). */
function nearestNode(graph, bbox, rows, cols, cell) {
  const c = cellCenter(bbox, rows, cols, cell.r, cell.c);
  let best = -1;
  let bestD = Infinity;
  for (let i = 0; i < graph.nodes.length; i++) {
    const n = graph.nodes[i];
    const d = (n.lat - c.lat) * (n.lat - c.lat) + (n.lon - c.lon) * (n.lon - c.lon);
...
/**
 * A real-road polyline for an ordered grid route. Each route cell is anchored to
 * the road vertex nearest its centre; consecutive anchors are joined by a short
 * path along the road graph. A pair with no short road connection (or when there
 * are no roads at all) falls back to the cell centre, so this always returns a
 * drawable polyline of `[lat, lon]` points. Pure — no DOM, no projection.
 */
export function routeGeometry(roads, bbox, rows, cols, cells, maxHops = 10) {
  const out = [];
  if (!Array.isArray(cells) || cells.length === 0) return out;
  const graph = buildRoadGraph(roads);
  if (graph.nodes.length === 0) return out;
  const push = (id) => {
    const n = graph.nodes[id];
    const last = out[out.length - 1];
    if (last && last[0] === n.lat && last[1] === n.lon) return;
    out.push([n.lat, n.lon]);
  };
  const anchors = cells.map((cell) => nearestNode(graph, bbox, rows, cols, cell));
  push(anchors[0]);
  for (let i = 1; i < anchors.length; i++) {
    const seg = bfsPath(graph, anchors[i - 1], anchors[i], maxHops);
    if (seg && seg.length > 1) {
      for (let k = 1; k < seg.length; k++) push(seg[k]);
    } else {
      const c = cellCenter(bbox, rows, cols, cells[i].r, cells[i].c);
      const last = out[out.length - 1];
      if (!last || last[0] !== c.lat || last[1] !== c.lon) out.push([c.lat, c.lon]);
    }
  }
  return out;
}
