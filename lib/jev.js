...
export async function runPolicyGame({
  ...
  while (!outcome) {
    ...
    // Nothing in the chain could be applied. Say WHY precisely: an unreadable
    // answer is 'unparsed'; a readable but unplayable first move is 'illegal'
    // (through a wall / off the board / into a warrior) or 'revisited' (legal
    // from here, but that cell was already walked).
    if (appliedHere === 0) {
      const first = chain[0];
      if (!first) {
        outcome = 'unparsed';
        ...
        const all = legalCandidates(board, ring, sector);
        const legal = all.some((c) => c.dir === first.dir);
        outcome = 'illegal';
        reject = legal ? 'revisited' : 'illegal';
        rejectDir = first.dir;
      }
      break;
    }
