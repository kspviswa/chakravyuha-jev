...
export async function runPolicyGame({
  ...
  return {
    ...
    // Was JEV's OWN proposed move right, step by step — true, false, or null
    // where it gave no usable answer at all. Positionally aligned with
    // confidenceBands, so the two together are the raw material for calibration.
    //
    // This is NOT the same as `stepFlags`. stepFlags grades the move APPLIED,
    // and since a red step always applies the correct move, stepFlags is true by
    // construction wherever we overruled. Calibrating on it would be circular:
    // low-confidence steps would score 100% precisely because we stopped
    // trusting them. Only Jev's own answers can say whether its confidence
    // predicts its accuracy.
    jevFlags: applied.map((a) => a.jevCorrect),
