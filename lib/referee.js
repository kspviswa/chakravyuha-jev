...
/**
 * Replay a direction list from the board's start. The ONLY place a move list
...
export function walkChakra(board, moves) {
  ...
  for (const m of moves) {
    ...
    if (nr !== 0 && nr !== ring && m === 'outward' && !openRadialAt(board, ring, sector)) { hitWall = true; break; }
...
export function chakraQuality(board) {
  ...
  // dist from the source, dist to the goal: a cell lies on some shortest route
  // iff distFromSource[u] + distToGoal[u] === optimal (all shortest paths).
  const key = (c) => `${c.ring},${c.sector}`;
  const bfs = (start) => {
  ...
  const fromSrc = bfs(board.src);
  const toGoal = bfs(board.dst);
  let warriorsOnOptimal = 0;
  for (const w of board.warriors || []) {
    const k = key(w);
    const a = fromSrc.get(k);
    const b = toGoal.get(k);
    if (Number.isFinite(a) && Number.isFinite(b) && a + b === s.length) warriorsOnOptimal++;
  }
