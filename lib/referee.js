...
function makeVerdict(board, moves, weighted) {
  ...
  // The walk's own fields are also exposed at the top level: callers that just
  // want "did it reach / how many steps / what did it cost" should not have to
  // reach through `.walk`, while the skins keep using the nested form.
  return {
    walk: w, optimal, weighted, checks, ok: checks.every((c) => c.pass),
    reached: w.reached, steps: w.steps, cost: w.cost,
    hitWall: w.hitWall, outOfBounds: w.outOfBounds, cells: w.cells,
  };
}
